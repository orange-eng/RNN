...
tf.app.flags.DEFINE_string('checkpoints_dir', os.path.abspath(path + './checkpoints/poems/'), 'checkpoints save path.')
tf.app.flags.DEFINE_string('file_path', os.path.abspath(path + './dataset/data/poems.txt'), 'file name of poems.')
...
def gen_poem(begin_word):
    ...
    with tf.Session() as sess:
        ...
        #checkpoint = tf.train.latest_checkpoint(FLAGS.checkpoints_dir)
        checkpoint = tf.train.latest_checkpoint(path+'./model/')
        #saver.restore(sess, checkpoint)
        saver.restore(sess, path+'./model/-24')
